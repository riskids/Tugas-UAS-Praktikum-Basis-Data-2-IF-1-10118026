...
<head>
    ...
    <title>Simpan Data Jurusan</title>
</head>
...
<nav class="navbar navbar-expand-lg  navbar-light bg-primary fixed-top">
    ...
    <div class="container">
        <h2 class="text-center m-3">Simpan Data jurusan Berhasil</h2>

        <?php
        include_once("DAO/function.php");
        $db = dbConnect();
        if ($db->connect_errno == 0) {
            $IdJurusan   = $db->escape_string($_POST["IdJurusan"]);
            $IdFakultas = $db->escape_string($_POST["IdFakultas"]);
            $NamaJurusan = $db->escape_string($_POST["NamaJurusan"]);

            $sql = "INSERT INTO jurusan(id_jurusan,id_fakultas,nama_jurusan)
        VALUES ('$IdJurusan','$IdFakultas','$NamaJurusan')";
            //echo $sql;
            $res = $db->query($sql);
            if ($res) {
        ...
        ?>
                <div class="text-center">
                    <a href="../Jurusan.php"><button class="btn btn-primary">Go</button></a>
                    </div>
                <?php
                } else {
                ?>
